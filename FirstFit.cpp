...
void firstFit(int pro[],int pro_size,int mem[],int mem_size){
	int pro_mem[pro_size][2],mem_pro[mem_size][3],sum_wait_pro=0,sum_rem_block=0,total_if=0,ef=0; 
	...
	for(int i=0;i<pro_size;i++){
		for(int j=0;j<mem_size;j++){
			if(mem_pro[j][1]==-1 && pro[i]<=mem[j]){
				pro_mem[i][1]=j;
				mem_pro[j][1]=pro[i];
				mem_pro[j][2]=mem[j]-pro[i];
				break;
			}
		}
	}
